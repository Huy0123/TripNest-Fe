...
export default function TourCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-grey-200 overflow-hidden hover:shadow-lg transition-shadow flex flex-col md:flex-row relative">
      <Link href={`/tour/${id}`} className="absolute inset-0 z-10">
        <span className="sr-only">Xem chi tiết {title}</span>
      </Link>
      {/* Image Section */}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between pt-4 border-t border-grey-100 gap-4">
            ...
            <div className="flex items-center justify-between md:justify-end gap-4 w-full md:w-auto">
                 ...
                 <div className="text-right">
                     ...
                     <span className="text-primary-600 header-05-bold">€{price != null ? new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(price) : '0.00'}</span>
                 </div>
            </div>
        </div>
      </div>
