      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6 lg:px-8 relative">
        ...
        {tours.length > 0 ? (
                  ...
                  <div className="group relative flex flex-col overflow-hidden rounded-2xl bg-white border border-gray-200 transition-all h-full">
                    <div className="relative h-[200px] w-full shrink-0 overflow-hidden">
                      <div
                        ...
                        style={{ backgroundImage: `url('${tour.image || tour.detail?.images?.[0]?.url || "https://images.unsplash.com/photo-1542314831-c6a4d14d23e5?q=80&w=600&auto=format&fit=crop"}')` }}
                      />
                    </div>
                    <div className="p-4 flex flex-col flex-grow bg-white">
                      <div>
                        ...
                        <div className="flex items-center gap-0.5 mb-1.5">
                          {[...Array(5)].map((_, i) => (
                            <svg key={i} className={`w-3.5 h-3.5 ${i < Math.floor((tour.rating || 0) / 2) ? 'text-[#f5c71a]' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                            </svg>
                          ))}
                        </div>
                        <div className="flex items-center gap-1 mb-1">
                          <span className="text-[13px] font-semibold text-[#1a73e8]">{tour.rating || 0}/10</span>
          ...
          <div className="w-full flex justify-center py-10 bg-gray-50 rounded-2xl border border-gray-100">
            <p className="text-gray-500 font-medium">Chưa có ưu đãi nào ở thời điểm hiện tại.</p>
          </div>
        )}
      </div>
    </section>
